"""
QUANTRA BEAST v3.1 — Production Server
...
DATA SOURCES (no ScraperAPI):
  - Kite WS/REST  → indices LTP, option LTP, OI enrichment
  - NSE direct    → OI chain (with proper session headers)
  - RSS feeds     → global + Indian market news (multi-source)
  - Stooq/Yahoo  → US futures, DXY, crude, SGX (global macro)
"""
...
# ─── NSE DIRECT SESSION (no ScraperAPI) ───────────────────────────────────────
_nse_session = None
_nse_session_ts = 0

def _get_nse_session():
    """Maintain a warm NSE session with valid cookies."""
    ...
    try:
        ...
        _nse_session = s  # use anyway
        _nse_session_ts = time.time()
    return s

def nse_get(path, retries=3):
    """
    Fetch from NSE API directly — no ScraperAPI.
    path: full URL like https://www.nseindia.com/api/...
    """
    s = _get_nse_session()
    for i in range(retries):
        try:
            ...
            if t.startswith("<") or "<!doctype" in t.lower():
                print(f"[NSE] HTML on attempt {i+1}, rebuilding session")
                _nse_session_ts = 0
                s = _get_nse_session()
                time.sleep(2 * (i + 1))
                continue
    ...
    try:
        ...
        if d.get("status") == "success":
            for key, val in d.get("data", {}).items():
                ...
                result[key] = {"ltp": ltp, "open": ohlc.get("open",0),
...
# ═══════════════════════════════════════════════════════════════════════════════
#  FETCH INDICES  (Kite primary, NSE fallback)
# ═══════════════════════════════════════════════════════════════════════════════
...
def fetch_indices():
    ...
    if kite["access_token"]:
        ...
        except Exception as e: print(f"[INDICES KITE] {e}")
    # NSE direct fallback
...
# ═══════════════════════════════════════════════════════════════════════════════
#  FETCH OPTION CHAIN  (NSE direct — no ScraperAPI)
# ═══════════════════════════════════════════════════════════════════════════════
...
def fetch_chain(symbol="NIFTY"):
    ...
    if not raw:
        return {"error": "failed", "chain": [], "spot": 0, "pcr": 1, "atm_strike": 0}
...
# ═══════════════════════════════════════════════════════════════════════════════
#  GLOBAL MACRO DATA  (free sources — impacts Indian markets)
# ═══════════════════════════════════════════════════════════════════════════════

def fetch_global_macro():
    """
    Fetch global indicators that directly impact Indian markets:
    - US Futures (S&P, Nasdaq, Dow) — from stooq
    - Crude Oil (WTI, Brent) — key for India (oil importer)
    - DXY (Dollar Index) — inverse to INR / FII flows
    - Gold — safe haven signal
    - SGX/Gift Nifty — pre-market India signal
    ...
    # ── Stooq tickers (free, no key) ──
    STOOQ_TICKERS = {
        "sp500_futures": "^spx",      # S&P 500
        "nasdaq_futures": "^ndx",     # Nasdaq 100
        "dow_futures": "^dji",        # Dow Jones
        "crude_wti": "cl.f",          # WTI Crude futures
        "crude_brent": "cb.f",        # Brent Crude
    ...
    def stooq_fetch(ticker):
        try:
            r = requests.get(
                f"https://stooq.com/q/l/?s={ticker}&f=sd2t2ohlcv&h&e=json",
                timeout=8
            )
            d = r.json()
            ...
            if symbols and symbols[0].get("close"):
                ...
                return {
                    ...
                    "change_pct": round(
                        (float(s.get("close", 0)) - float(s.get("open", 0))) /
                        float(s.get("open", 1)) * 100, 2
                    ),
                    "source": "stooq"
                }
        except: pass
...
# ═══════════════════════════════════════════════════════════════════════════════
#  MULTI-SOURCE NEWS  (global + Indian, impacts Indian markets)
# ═══════════════════════════════════════════════════════════════════════════════

# News sources — all free RSS/JSON
NEWS_SOURCES = [
    # ── Indian Markets (direct) ──
    {
        "name": "Economic Times Markets",
        "url":  "https://economictimes.indiatimes.com/markets/rssfeeds/1977021501.cms",
        "type": "rss", "region": "IN", "weight": 1.0
    },
    {
        "name": "Moneycontrol",
    ...
    # ── RBI / Policy (high impact) ──
    {
        "name": "RBI Press",
        "url":  "https://rbi.org.in/scripts/RSS.aspx?Id=316",
        "type": "rss", "region": "IN_POLICY", "weight": 1.5
    },
    # ── Global — US Fed / Economy ──
    {
        "name": "Reuters Business",
        "url":  "https://feeds.reuters.com/reuters/businessNews",
        "type": "rss", "region": "GLOBAL", "weight": 1.2
    },
    {
    ...
    },
    {
        ...
        "type": "rss", "region": "GLOBAL", "weight": 1.0
    },
    {
        "name": "FT Markets",
        "url":  "https://www.ft.com/markets?format=rss",
        "type": "rss", "region": "GLOBAL", "weight": 1.1
    },
    # ── Global Macro — geopolitical ──
    {
        "name": "BBC Business",
        "url":  "http://feeds.bbci.co.uk/news/business/rss.xml",
        "type": "rss", "region": "GLOBAL", "weight": 0.8
    },
    # ── NSE corporate announcements ──
    {
        "name": "NSE Announcements",
        "url":  "https://www.nseindia.com/api/corporate-announcements?index=equities",
        "type": "nse_json", "region": "IN_CORP", "weight": 0.8
    },
]

# Extended signal keywords — Indian market context
NEWS_SIGNALS = {
    # ── Bullish — Indian policy ──
    "rate cut": 85, "repo cut": 85, "rbi cut": 85, "rbi easing": 75,
    "stimulus": 70, "gdp beat": 65, "gdp growth": 65, "inflation ease": 60,
    "cpi lower": 60, "cpi fall": 60, "wpi lower": 55, "trade surplus": 55,
    "current account surplus": 55, "buyback": 55, "dividend": 50,
    "upgrade": 55, "rating upgrade": 65, "record high": 50, "all time high": 55,
    "profit surge": 55, "earnings beat": 55, "strong results": 55,
    "fii buy": 70, "fii inflow": 70, "dii buy": 55, "foreign inflow": 65,
    "fdi": 60, "dovish": 60, "easing": 55, "rate pause": 35,
    "rate hold": 30, "liquidity": 45, "open market operation": 50,
    # ── Bullish — Global (impacts India) ──
    "fed pause": 60, "fed hold": 55, "fed pivot": 70, "fed cut": 75,
    "soft landing": 60, "us jobs strong": 50, "dollar weak": 55, "dxy fall": 55,
    "crude fall": 65, "oil fall": 65, "crude drop": 65, "oil price drop": 65,
    "china stimulus": 55, "global rally": 60, "risk on": 55,
    "us market up": 50, "nasdaq up": 50, "s&p up": 50, "dow up": 45,
    # ── Bearish — Indian policy ──
    "rate hike": -85, "repo hike": -85, "rbi hike": -85, "hawkish": -70,
    "recession": -80, "inflation surge": -70, "cpi high": -65, "cpi spike": -65,
    "wpi high": -55, "trade deficit": -50, "current account deficit": -50,
    "downgrade": -65, "rating cut": -70, "loss": -50, "profit fall": -55,
    "earnings miss": -55, "revenue drop": -50, "layoffs": -40,
    "bankruptcy": -70, "default": -75, "crisis": -70, "sell off": -65,
    "fii sell": -75, "fii outflow": -75, "capital outflow": -65, "outflow": -60,
    "slowdown": -55, "contraction": -60, "tax hike": -55,
    # ── Bearish — Global (impacts India) ──
    "fed hike": -80, "fed hawkish": -75, "rate rise": -70,
    "us recession": -75, "us slowdown": -60, "dollar strong": -55,
    "dxy rise": -55, "crude surge": -60, "oil spike": -65, "oil high": -55,
    "crude up": -55, "war": -80, "conflict": -70, "geopolitical": -50,
    "sanctions": -60, "tariff": -55, "trade war": -65, "china slowdown": -55,
    "global sell off": -70, "risk off": -65, "market crash": -80,
    "us market down": -50, "nasdaq down": -55, "s&p down": -55,
    "yield surge": -60, "bond sell off": -55, "vix spike": -70,
}
...
def _parse_nse_json(url, source_name, weight):
    """Parse NSE corporate announcements JSON."""
    items = []
    try:
        d = nse_get(url)
        if d and isinstance(d, list):
            for x in d[:15]:
                items.append({
                    "title":   x.get("subject", "") or x.get("desc", ""),
                    "desc":    "",
                    "time":    x.get("an_dt", "") or x.get("exchdisstime", ""),
                    "source":  source_name,
                    "weight":  weight,
                    "company": x.get("symbol", "")
                })
    except Exception as e:
        print(f"[NSE JSON {source_name}] {e}")
    return items
...
def fetch_all_news():
    ...
    def fetch_source(src):
        try:
            if src["type"] == "rss":
                return _parse_rss(src["url"], src["name"], src["weight"])
            elif src["type"] == "nse_json":
                return _parse_nse_json(src["url"], src["name"], src["weight"])
        except Exception as e:
            errors.append(f"{src['name']}: {e}")
        return []

    threads = []
    results = [[] for _ in NEWS_SOURCES]

    def worker(i, src):
        results[i] = fetch_source(src)

    for i, src in enumerate(NEWS_SOURCES):
        t = threading.Thread(target=worker, args=(i, src), daemon=True)
        threads.append(t)
        t.start()

    # Wait max 12s for all sources
    for t in threads:
        t.join(timeout=12)
...
def engine_options(oc):
    ...
    return {"name": "Options", "score": round(sc, 1),
            ...
            "data": {"pcr": pcr, "max_pain": mp, "skew": skew}}

def engine_gamma(oc):
    chain = oc.get("chain", []); atm = oc.get("atm_strike", 0); sc = 0; rs = []
    if not chain:
        return {"name": "Gamma", "score": 0, "signal": "NEUTRAL", "confidence": 0,
                "reasons": ["No chain data"], "data": {}}
    ...
    if above and below:
        ac = max(above, key=lambda x: x.get("ce_oi", 0))
        bp = max(below, key=lambda x: x.get("pe_oi", 0))
        if ac.get("ce_oi", 0) > bp.get("pe_oi", 0) * 1.5:
            sc -= 15; rs.append(f"CE wall at {ac['strike']}")
        elif bp.get("pe_oi", 0) > ac.get("ce_oi", 0) * 1.5:
            sc += 15; rs.append(f"PE wall at {bp['strike']}")
    sc = max(-100, min(100, sc))
...
def engine_volatility(idx, oc):
    ...
    vc  = idx.get("INDIA VIX", {}).get("pChange", 0); sc = 0; rs = []
...
def engine_news(na):
    ns = na.get("news_sentiment_score", 0)
    bc = na.get("bull_count", 0); rc = na.get("bear_count", 0)
    ...
    return {"name": "News", "score": round(sc, 1),
            "signal": "BULL" if sc > 15 else "BEAR" if sc < -15 else "NEUTRAL",
            "confidence": min(95, abs(sc)), "reasons": rs,
            "data": {"sentiment": ns, "bull": bc, "bear": rc}}
...
def generate_signal(snapshot):
    ...
    na  = snapshot.get("news_analysis", {"news_sentiment_score": 0, "bull_count": 0,
                                          "bear_count": 0, "impactful_news": []})
    e = {"trend": engine_trend(idx, oc), "options": engine_options(oc),
         "gamma": engine_gamma(oc), "volatility": engine_volatility(idx, oc),
         "regime": engine_regime(idx, oc), "sentiment": engine_sentiment(fii),
         "flow": engine_flow(oc), "news": engine_news(na)}
    ...
    reasons = [e[k]["reasons"][0] for k in ["options", "sentiment", "flow", "trend",
               "gamma", "volatility", "regime", "news"] if e[k]["reasons"]]
    ...
    return {"signal_id": str(uuid.uuid4())[:8], "timestamp": datetime.datetime.now().isoformat(),
            ...
            "engines": {k: {"score": v["score"], "signal": v["signal"],
                            "confidence": v["confidence"]} for k, v in e.items()},
...
class CapMgr:
    ...
    def can_trade(self):
        self._chk_day()
        if self.ses_trades >= self.max_trades:
            return {"allowed": False, "reason": f"Max {self.max_trades} trades"}
        dlim = self.total * 0.03
        if self.ses_pnl <= -dlim:
            return {"allowed": False, "reason": f"Daily loss ₹{dlim:.0f} hit"}
        dd = (self.peak - self.avail) / self.peak * 100 if self.peak > 0 else 0
        if dd >= 10:
            return {"allowed": False, "reason": f"Drawdown {dd:.1f}% — paused"}
    ...
    def status(self):
        ...
        return {"total_capital": self.total, "available_capital": round(self.avail, 2),
                ...
                "consecutive_loss": self.consec, "drawdown_pct": dd,
                "daily_loss_used_pct": dlp,
...
def _do_generate_signal(cfg):
    ...
    if "error" in oc:
        return None, None, None, {"error": f"Data fetch failed: {oc['error']}"}
...
@app.route("/")
def root():
    return jsonify({"status": "alive", "version": "3.1", "service": "QUANTRA BEAST",
...
def trade_enter():
    ...
    return jsonify({"trade_id": tid, "message": "Trade recorded",
                    ...
                    "kite_orders_placed": place_orders,
                    "kite_order_results": order_results})
...
def trade_monitor():
    ...
    if ((action == "BUY CE" and ce_s > 0 and ce_s <= ce_sl) or
            (action == "BUY PE" and pe_s > 0 and pe_s <= pe_sl)):
...
def trade_exit():
    ...
    risk_mgr.record(pnl)
    if active_tid:
        close_trade_db(active_tid, {"pnl": pnl, "exit_type": etype,
                                     "capital_after": risk_mgr.avail})
...
def global_macro_route():
    """Global indicators — US futures, crude, DXY, gold, USD/INR"""
...
def market_news():
    """Multi-source news — Indian + global, scored for Indian market impact"""
    ...
    result = {
        "news": news_raw[:30],
        "impactful": na["impactful_news"],
        "sentiment": na["news_sentiment_score"],
        "bull_count": na["bull_count"],
        "bear_count": na["bear_count"],
        "total_analyzed": na["total_news_analyzed"],
        "sources": na.get("sources_used", []),
