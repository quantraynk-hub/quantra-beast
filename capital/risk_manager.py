"""
CAPITAL MANAGEMENT ENGINE — Stage 5
...
class RiskManager:
    def __init__(self, config: dict):
        ...
        self.max_trades_day   = config.get("max_trades_day", 3)
    ...
    def can_trade(self) -> dict:
        ...
        if self.session_trades >= self.max_trades_day:
            return {"allowed": False, "reason": f"Max trades ({self.max_trades_day}) reached for today"}

        # Daily loss limit
        daily_loss_limit = self.total_capital * (self.max_daily_loss / 100)
        if self.session_pnl <= -daily_loss_limit:
            return {"allowed": False, "reason": f"Daily loss limit ₹{daily_loss_limit:.0f} hit — no more trades today"}

        # Drawdown protection
        current_drawdown = (self.peak_capital - self.available_capital) / self.peak_capital * 100
        if current_drawdown >= self.max_drawdown:
            return {"allowed": False, "reason": f"Drawdown {current_drawdown:.1f}% exceeds {self.max_drawdown}% limit — system paused"}

        # 2 consecutive losses → reduce risk (handled in position size, not blocked)
    ...
    def calculate_position(self, signal: dict, option_data: dict) -> dict:
        ...
        # Risk amount — reduce by 50% if 2 consecutive losses
